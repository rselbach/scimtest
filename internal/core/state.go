...
const (
	// SAMLEncryptionAlgorithmAES128GCM selects AES-128-GCM assertion encryption.
	SAMLEncryptionAlgorithmAES128GCM = "aes128-gcm"
	// SAMLEncryptionAlgorithmAES192GCM selects AES-192-GCM assertion encryption.
	SAMLEncryptionAlgorithmAES192GCM = "aes192-gcm"
	// SAMLEncryptionAlgorithmAES256GCM selects AES-256-GCM assertion encryption.
	SAMLEncryptionAlgorithmAES256GCM = "aes256-gcm"
	// DefaultSAMLEncryptionAlgorithm is used when no algorithm is configured.
	DefaultSAMLEncryptionAlgorithm = SAMLEncryptionAlgorithmAES256GCM
)

// NormalizeSAMLEncryptionAlgorithm trims value and defaults an empty value to
// AES-256-GCM.
func NormalizeSAMLEncryptionAlgorithm(value string) string {
	value = strings.TrimSpace(value)
	if value == "" {
		return DefaultSAMLEncryptionAlgorithm
	}
	return value
}

// ValidateSAMLEncryptionAlgorithm rejects unknown assertion ciphers.
func ValidateSAMLEncryptionAlgorithm(value string) error {
	switch NormalizeSAMLEncryptionAlgorithm(value) {
...
//
// Whitespace-only PEM means do not encrypt. A currently valid RSA CERTIFICATE
// means encrypt to that key. Anything else is an error.
func ParseSAMLEncryptionCertificate(value string) (*x509.Certificate, error) {
	if strings.TrimSpace(value) == "" {
		return nil, nil
	}
	cert, err := parseSAMLCertificate(value)
	if err != nil {
		return nil, fmt.Errorf("SAML encryption certificate is invalid: %w", err)
	}
	return cert, nil
}
