...
func TestSAMLResponseMatchesAuthnRequest(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			r.Contains(body, `id="app-shell"`)
			r.Contains(body, `<h1>SAML Inspector</h1>`)
			r.Contains(body, `class="side-item active" href="/?environment=app-1&amp;tab=saml-inspector"`)
			r.Contains(body, `name="return_tab" value="saml-inspector"`)
			r.NotContains(body, `name="fault_token_error"`)
