...
func TestDashboardRendersCriticalFlowAffordances(t *testing.T) {
	...
	r.Equal(1, strings.Count(body, ">Fault Injection</a>"))
...
func TestAutomaticTunnelRetry(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/config/tunnel/retry", strings.NewReader(form.Encode()))
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	rec := httptest.NewRecorder()
...
func TestAutomaticTunnelRetryIsPostOnly(t *testing.T) {
	...
	newTestIDPApp(t).routes().ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/config/tunnel/retry", nil))
